...
```javascript
import Vue from 'vue'
import CiicStep from 'element-steps'
...
```javascript
import Vue from 'vue'
import { CiicSteps, CiicStep } from 'element-steps'

Vue.component('el-steps', CiicSteps)
Vue.component('el-step', CiicStep)
```
...
| icon | 图标 | Ciicement Icon 提供的图标，如果要使用自定义图标可以通过 slot 方式写入 | string | — |
